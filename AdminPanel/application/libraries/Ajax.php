...
class Prototype extends JavaScript {
	...
	function submit_to_remote($name,$value,$options)
  {
	...
	function submit_class_to_remote($name,$value,$class,$options)
  {
		if(isset($options['with']))
    {
			$options['with'] ="Form.serialize(this.form)";
		...
		return '<input type="button" class="'.$class.'" onclick="'.$this->remote_function($options).'" name="'.$name.'" value ="'.$value.'" />';
	}
