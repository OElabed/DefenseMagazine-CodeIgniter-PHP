...
class  Client_model extends CI_Model {
	...
	function get_client($id_client){
		...
		$client_tr['civil_client'] = $row->civil_client;
		$client_tr['adresse_client'] = $row->adresse_client;
	...
	function ajouter_client($client_aj){
		...
		$client = array(
				   ...
				   'pays_monnaie' => $client_aj['pays_monnaie'] 
					);
	...
	function exist_client_mod($email,$id){
	...
	function client_inscrie($email,$pass){
		$pass=do_hash($pass);
		$query_str = "SELECT email_client FROM client WHERE email_client = '$email' and mot_pass_client = '$pass' ";
		$result = $this->db->query($query_str);
